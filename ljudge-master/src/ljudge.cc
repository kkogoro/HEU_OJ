...
#define LJUDGE_VERSION "v0.6.1_heu_noip"
...
string string_chomp(const string& str) {
	string in = str;
	if (in.length() && in[in.length() - 1] == EOF)
		in.pop_back();
	while (in.length() && in[in.length() - 1] == ' ')
		in.pop_back();
	while (in.length() && in[in.length() - 1] == '\n')
		in.pop_back();
	in += "\n";
...
static void run_standard_checker(j::object& result, const Testcase& testcase, const string& user_output_path) {
	...
	use_sha1 = false; //To prevent PE problem, now it's forced to be 'false'. (by techiah)
	...
	if (use_sha1) {
		if (sha1(usr) == testcase.output_sha1) {
			...
			result["result"] = j::value(TestcaseResult::ACCEPTED);//force PE to AC
		}
		else {
